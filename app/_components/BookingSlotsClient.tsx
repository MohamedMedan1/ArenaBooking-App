...
export default function BookingSlotsClient({ field }: { field: any }) {
  ...
  const activeDateData = timeSlots.find(
    (ts: any) => String(ts.date) === selectedDate,
  );
          ...
          >
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] uppercase tracking-[0.2em] font-semibold text-foreground/50">
                  Live Availability
                </span>
              </div>
              <h1 className="text-4xl font-bold tracking-tight">
                {name}{" "}
                <span className="text-foreground/30 font-light italic">
                  / Book Session
                </span>
              </h1>
            </div>
            <div className="text-right hidden md:block">
              <p className="text-xs text-foreground/40 uppercase tracking-widest mb-1 font-semibold">
                Location
              </p>
          ...
          <motion.div variants={itemVariants} className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-foreground/30">
                {activeDate?.month} {activeDate?.date.getFullYear()}
              </span>
            </div>
            ...
            <div className="flex gap-3 overflow-x-auto pb-4 no-scrollbar -mx-4 px-4 md:mx-0 md:px-0">
              {DATES.map((date: any) => (
                ...
                >
                  <span
                    className={`text-[10px] uppercase font-bold ${selectedDate === date.id ? "opacity-70" : "text-foreground/30 group-hover:text-foreground/60"}`}
                  >
                    {date.day}
                  </span>
                  <span
                  ...
                  <span
                    className={`text-[10px] uppercase font-bold ${selectedDate === date.id ? "opacity-70" : "text-foreground/30"}`}
                  >
                    {date.month}
                  </span>
                </button>
                    ...
                    {periodSlots.map((slot: any) => (
                      ...
                      >
                        <div className="flex flex-col gap-0.5">
                          <span
                            className={`text-[10px] font-mono tracking-tighter ${selectedSlotId === slot.id ? "text-foreground" : "text-foreground/40"}`}
                          >
                            {slot.time}
                          </span>
                        ...
                        <div className="mt-2 flex items-center justify-between">
                          <span
                            className={`text-xs font-semibold ${!slot.isAvailable ? "text-foreground/40" : "text-primary"}`}
                          >
                            {slot.isAvailable
                              ? `$${slot.price}.00`
                              : "Reserved"}
            ...
            <div className="relative h-full flex flex-col">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-bold text-sm">
                        {activeDate
                          ? `${activeDate.month} ${activeDate.label}, ${activeDate.date.getFullYear()}`
                          : "Select Date"}
                      </p>
                      <p className="text-xs text-foreground/60">
                        {activeSlot ? `${activeSlot.time} - 1.5h` : "--:--"}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-auto pt-6 flex flex-col gap-4">
                ...
                <div className="text-[10px] text-center text-foreground/30 px-4 leading-relaxed font-medium">
                  ...
                  <span className="underline decoration-border-alpha hover:text-foreground/60 cursor-pointer">
                    Cancellation Policy
                  </span>{" "}
                  and Arena rules.
                </div>
